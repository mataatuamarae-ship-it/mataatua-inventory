...
// Loads the Supabase library from a CDN in the background, with a timeout,
// so a slow/blocked network never holds up the app itself — everything
// above (search, add/edit, +/-, print) works the instant the page renders,
...
async function seedIfEmpty() {
  ...
  const { data, error } = await supabase.from("inventory_items").insert(seed).select("*");
  if (error) throw error;
  return data;
}
...
async function flushQueue() {
  ...
  for (const op of q) {
    try {
      if (op.type === "upsert") {
        const { error } = await supabase.from("inventory_items").upsert(op.row);
        if (error) throw error;
      } else if (op.type === "delete") {
        const { error } = await supabase.from("inventory_items").delete().eq("id", op.id);
        if (error) throw error;
      }
    } catch (e) {
      remaining.push(op);
...
async function saveItem(row) {
  ...
  setStatus(supabase ? "offline — will sync" : "offline (not configured)", "offline");
}
